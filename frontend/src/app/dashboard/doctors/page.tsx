...
const Doctors: React.FC = () => {
  ...
  // Carregar médicos quando a página for montada
  useEffect(() => {
    fetchDoctors();
  }, []);
  ...
  const handleSaveDoctor = async (data: FormData) => {
    setIsLoading(true);
    ...
    try {
      ...
      await fetchDoctors(); // Recarrega a lista de médicos
      closeModal();
    } catch (error) {
            ...
            <>
              ...
              <div className={styles.pagination}>
                <div className={styles.paginationInfo}>
                  Mostrando <strong>1-{filteredDoctors.length}</strong> de{" "}
                  <strong>{filteredDoctors.length}</strong> médicos
                </div>
        ...
        <DeleteConfirmationModal
          ...
          message={
            <>
              ...
              <strong>{currentDoctor.nome}</strong>. Esta ação não pode ser
              desfeita.
            </>
          }
          isLoading={isLoading}
